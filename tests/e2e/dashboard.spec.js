...
test.describe('Panglong ERP - Dashboard', () => {
  ...
  test('super admin dashboard shows platform metrics', async ({ page }) => {
    const consoleErrors = [], pageErrors = [];
    page.on('console', msg => { if (msg.type() === 'error') consoleErrors.push(msg.text()); });
    page.on('pageerror', err => pageErrors.push(err.message));
    ...
    await expect(page.locator('.card-title:has-text("Total Tenants")')).toBeVisible();
    await expect(page.locator('.card-title:has-text("Total Revenue")')).toBeVisible();
    await expect(page.locator('.card-header:has-text("Tenant Terbaru")')).toBeVisible();
