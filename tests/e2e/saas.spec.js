...
test.describe('Panglong ERP - SaaS Management Page', () => {
  ...
  test('saas dashboard shows revenue cards', async ({ page }) => {
    const consoleErrors = [], pageErrors = [];
    page.on('console', msg => { if (msg.type() === 'error') consoleErrors.push(msg.text()); });
    page.on('pageerror', err => pageErrors.push(err.message));
    await page.goto(`${FRONTEND_BASE}/saas.php`);
    await page.waitForLoadState('networkidle');
    await expect(page.locator('h1')).toContainText('SaaS Management');
    await expect(page.locator('.card-body:has-text("Total Tenant")')).toBeVisible();
    await expect(page.locator('.card-body:has-text("Pendapatan Bulan Ini")')).toBeVisible();
    expect(consoleErrors).toEqual([]);
    expect(pageErrors).toEqual([]);
  });

  test('saas invoices tab works', async ({ page }) => {
    const consoleErrors = [], pageErrors = [];
    page.on('console', msg => { if (msg.type() === 'error') consoleErrors.push(msg.text()); });
    page.on('pageerror', err => pageErrors.push(err.message));
    await page.goto(`${FRONTEND_BASE}/saas.php?tab=invoices`);
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.nav-tabs .nav-link.active')).toContainText('Faktur');
